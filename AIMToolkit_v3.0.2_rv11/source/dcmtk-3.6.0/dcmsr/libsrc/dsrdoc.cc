        ...
        if (!(flags & PF_printNoDocumentHeader))
        {
            ...
            if (!PatientName.isEmpty())
            {
                ...
                if (!EthnicGroup.isEmpty())
                {
                   if (!patientStr.empty())
                       patientStr += ", ";
                   patientStr += getPrintStringFromElement(EthnicGroup, tmpString);
                }
            ...
            /* manufacturer, device and software versions */
            if (!Manufacturer.isEmpty())
            {
                stream << "Manufacturer: Serial Number: Software Version  : " << getPrintStringFromElement(Manufacturer, tmpString);
                OFString deviceStr;
                ...
                if (!SoftwareVersions.isEmpty())
                {
                   if (!deviceStr.empty())
                       deviceStr += ", ";
                   deviceStr += '#';
                    deviceStr += getPrintStringFromElement(SoftwareVersions, tmpString);
                }
                if (!deviceStr.empty())
                    stream << " (" << deviceStr << ")";
                stream << OFendl;
                                  ...
                                  const size_t flags)
{
    ...
    if (isValid())
    {
        ...
        if ((flags & XF_writeEmptyTags) || !EthnicGroup.isEmpty())
            ...
            if (!Manufacturer.isEmpty())
            {
                ...
                if (!SoftwareVersions.isEmpty())
                {
                   if (!deviceStr.empty())
                       deviceStr += ", ";
                   deviceStr += '#';
                   deviceStr += convertToHTMLString(getStringValueFromElement(SoftwareVersions, tmpString), htmlString, newFlags);
                }
                if (!deviceStr.empty())
                    stream << " (" << deviceStr << ")";
                stream << "</td>" << OFendl;
...
const char *DSRDocument::getEthnicGroup() const
{
    return getStringValueFromElement(EthnicGroup);
}
...
const OFString &DSRDocument::getEthnicGroup(OFString &value) const
{
    return getStringValueFromElement(EthnicGroup, value);
}
...
{
    /* might add check for correct format (VR) later on */
    return EthnicGroup.putString(value.c_str());
}
